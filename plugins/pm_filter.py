        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await auto_filter(client, message.text, message, None, ai_search)
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Searching...🔍</i></b>", reply_to_message_id=message.id)
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
InlineKeyboardButton("⚓ 𝗟𝗮𝘁𝗲𝘀𝘁 𝗠𝗼𝘃𝗶𝗲 𝗨𝗽𝗱𝗮𝘁𝗲𝘀 ⚓", url=f"https://t.me/+mhBXTSeCRsM1ZjY1")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("⚙️ 𝗕𝗼𝘁 𝗨𝗽𝗱𝗮𝘁𝗲 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 ⚙️", url=f"https://t.me/TBM_Bot_Updates")           
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>Searching...</i></b>")
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/MrTechMovies_Group")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Bᴏᴛ Eᴅɪᴛᴇʀ', url="https://t.me/MR_BOT_EDITER")
        ],[
            InlineKeyboardButton('Hᴏᴍᴇ', callback_data='start'),
            InlineKeyboardButton('Cʟᴏsᴇ', callback_data='close_data')
        ]]
...
InlineKeyboardButton("⚓ 𝗟𝗮𝘁𝗲𝘀𝘁 𝗠𝗼𝘃𝗶𝗲 𝗨𝗽𝗱𝗮𝘁𝗲𝘀 ⚓", url=f"https://t.me/+mhBXTSeCRsM1ZjY1")
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>REQUESTED BY {message.from_user.mention}</b>"
        else:
            cap = f"<b>REQUESTED BY {message.from_user.mention}</b>"
